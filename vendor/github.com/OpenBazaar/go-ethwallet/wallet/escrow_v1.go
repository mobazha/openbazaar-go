...
func (_Escrow *EscrowFilterer) FilterFundAdded(opts *bind.FilterOpts, scriptHash [][32]byte, from []common.Address) (*EscrowFundAddedIterator, error) {
	...
	logs, sub, err := _Escrow.contract.FilterLogs(opts, "FundAdded", scriptHashRule, fromRule)
	if err != nil {
		return nil, err
	}
...
//
// Solidity: event FundAdded(bytes32 indexed scriptHash, address indexed from, uint256 valueAdded)
func (_Escrow *EscrowFilterer) WatchFundAdded(opts *bind.WatchOpts, sink chan<- *EscrowFundAdded, scriptHash [][32]byte, from []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _Escrow.contract.WatchLogs(opts, "FundAdded", scriptHashRule, fromRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
